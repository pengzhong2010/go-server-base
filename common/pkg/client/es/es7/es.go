...
//type IndexMappingConf struct {
//	IndexMapping map[string]struct {
...
func New() (client *EsClient, cf func(), err error) {
	Client.EsURL = strings.Split(conf.ES_URL, ",")
	if len(Client.EsURL) < 1 {
		err = fmt.Errorf("no es url provided")
		return
	}
	Client.Client, err = NewEs7(Client.EsURL...)
	if err != nil {
		return
	}
	cf = func() {}
...
func NewEs7(es7URL ...string) (client *elastic.Client, err error) {
	client, err = elastic.NewClient(
		elastic.SetURL(es7URL...),
		elastic.SetSniff(false),
	)
	if err != nil {
		log.Error("创建es7URL客户端异常")
		return
	}
	return
}
func (t *EsClient) Ping(c context.Context) {
	info, code, err := t.Client.Ping(t.EsURL[0]).Do(c)
	if err != nil {
		log.Error("error")
		return
	}
	log.Info("Elasticsearch returned with code %d and version %s\n", code, info.Version.Number)
}
func (t *EsClient) ExistIndex(c context.Context, indexName string) (exists bool, err error) {
	exists, err = t.Client.IndexExists(indexName).Do(c)
	return
}
...
func (t *EsClient) CreateIndex(c context.Context, indexName string, mapping string) (err error) {
	...
	createIndex, errE1 := t.Client.CreateIndex(indexName).Body(mapping).Do(c)
	if errE1 != nil {
...
func (t *EsClient) DeleteIndex(c context.Context, indexName string) (err error) {
	...
	deleteIndex, err := t.Client.DeleteIndex(indexName).Do(c)
	if err != nil {
...
func (t *EsClient) AddData(c context.Context, indexName string, id int64, data interface{}) (err error) {
	_, err = t.Client.Index().Index(indexName).Id(strconv.FormatInt(id, 10)).BodyJson(data).Do(c)
	if err != nil {
...
func (t *EsClient) UpdateData(c context.Context, indexName string, id int64, data interface{}) (err error) {
	_, err = t.Client.Update().Index(indexName).Id(strconv.FormatInt(id, 10)).Doc(data).Do(c)
	if err != nil {
...
func (t *EsClient) Refresh(c context.Context, indexName string) (err error) {
	_, err = t.Client.Refresh().Index(indexName).Do(c)
	if err != nil {
...
func (t *EsClient) AddAliasIndex(c context.Context, indexName, aliasName string) (err error) {
	...
	res, errE1 := t.Client.Alias().Add(indexName, aliasName).Do(c)
	if errE1 != nil {
...
func (t *EsClient) RemoveAliasIndex(c context.Context, indexName, aliasName string) (err error) {
	...
	res, errE1 := t.Client.Alias().Remove(indexName, aliasName).Do(c)
	if errE1 != nil {
